...
def _my_dequantize_bf16(W, quant_state):
    ...
    # my_out_absmax = my_cdequantize_blockwise_fp32(code2, absmax, absmax2, blocksize2, n_elements_absmax)
    ...
    # my_out = cdequantize_blockwise_bf16_nf4(W, out_absmax, blocksize, out.numel(), out.shape)
    # print("Close: ", torch.allclose(out, my_out))
...
def cdequantize_blockwise_bf16_nf4(A, absmax, blocksize, n, shape):
    ...
    for block in range(A.numel() // (blocksize // 2)):
        for j in range(blocksize // 2):
            my_out[2 * ((block * (blocksize//2)) + j)] = nf4_data[(int(A[(block * (blocksize//2)) + j]) >> 4)] * absmax[block]
            my_out[2 * ((block * (blocksize//2)) + j) + 1] = nf4_data[int(A[(block * (blocksize//2)) + j]) & 0x0F] * absmax[block]
